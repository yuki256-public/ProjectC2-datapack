...
#発動対象がプレイヤーの場合
execute store success score @s counter if entity @s[team=Red] run scoreboard players operation @e[sort=nearest,limit=1,tag=PSCount] playerNumber = @a[team=Blue,distance=..5,limit=1,nbt={HurtTime:10s},tag=Battle]
execute store success score @s counter if entity @s[team=Blue] run scoreboard players operation @e[sort=nearest,limit=1,tag=PSCount] playerNumber = @a[team=Red,distance=..5,limit=1,nbt={HurtTime:10s},tag=Battle]
